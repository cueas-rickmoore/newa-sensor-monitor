...
parser.add_option('-e', action='store', type='string', dest='elements',
                  default='all')
parser.add_option('-m', action='store', type='int', dest='min_run_length',
                  default=None)
parser.add_option('-r', action='store_true', dest='report_missing',
                  default=False)
...
for station in stations:
    ...
    if not debug:
        log_filepath = factory.getFilepathForUcanid(ucanid, 'sequences')
    ...
    try:
        historicalSequences(factory, ucanid, elements, options.report_missing,
